...
# sensing module Ai and Av from EMMA, reach github for model weights
...
# model = UNet5()
# model.load_state_dict(torch.load("model/Ai.pth"))
# device = 'cuda' if torch.cuda.is_available() else 'cpu'
# model.to(device)
# model.eval()

# path_ir = "/data/ykx/MSRS_test/ir"
# path_vi = "/data/ykx/MSRS_test/vi"
# path_result = './result/msrs'


# with torch.no_grad():
#     for imgname in tqdm(os.listdir(path_result)):
#         if imgname == '3.jpg':
#             IR = cv2.imread(os.path.join(path_result, imgname), cv2.IMREAD_GRAYSCALE)[np.newaxis,np.newaxis,...].astype(np.float32) / 255.0
#             IR = torch.from_numpy(IR).to(device)
#             rec_ir = model(IR)
#             rec_ir = (rec_ir * 255).squeeze().squeeze().cpu().numpy()
#             plt.imshow(rec_ir, cmap='gray')
#             plt.show()
#             break