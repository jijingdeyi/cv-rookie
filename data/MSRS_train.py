...
def rgb2y(img):
    # img is rgb format, [3, h, w], float32
    y = (
        img[0:1, :, :] * 0.299000
        + img[1:2, :, :] * 0.587000
        + img[2:3, :, :] * 0.114000
...
IR_files = sorted(get_img_file(r"dataprocessing\MSRS_train\ir"))
VIS_files = sorted(get_img_file(r"dataprocessing\MSRS_train\vi"))

assert len(IR_files) == len(VIS_files)
h5f = h5py.File(
    os.path.join(
        ".\\data", data_name + "_" + str(img_size) + "_" + str(stride) + ".h5"
    ),
    "w",
)
...
for i in tqdm(range(len(IR_files))):
    ...
    # crop
    I_IR_Patch_Group = Im2Patch(I_IR, img_size, stride) # (1, 128, 128, 12)
    I_VIS_Patch_Group = Im2Patch(I_VIS, img_size, stride)  # (3, 128, 128, 12)
    ...
    for ii in range(I_IR_Patch_Group.shape[-1]):
        ...
        if not (bad_IR or bad_VIS):
            ...
            avl_IR = avl_IR[None, ...] # [1, 128, 128]
            avl_VIS = avl_VIS[None, ...] # [1, 128, 128]
...
with h5py.File(
    os.path.join("data", data_name + "_" + str(img_size) + "_" + str(stride) + ".h5"),
    "r",
) as f:
    for key in f.keys():
