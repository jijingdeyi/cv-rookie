...
class RandomCropPair:
    ...
    def __call__(self, vis_img, ir_img, fuse_image):
        ...
        vis_img = F.to_tensor(vis_img)  # type: ignore
        ir_img = F.to_tensor(ir_img)  # type: ignore
        fuse_image = F.to_tensor(fuse_image)  # type: ignore
    ...
    def __getitem__(self, item):
        ...
        if self.transform is not None:
            if isinstance(self.transform, RandomCropPair):
                image_A, image_B, other_fuse = self.transform(
                    image_A, image_B, other_fuse
                )
            else:
