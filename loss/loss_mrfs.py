...
class FusionLoss(nn.Module):
    ...
    def forward(self, input_xy, output, Mask):
        ...
        Fuse = output * Mask
        YCrCb_Fuse = RGB2YCrCb(Fuse)
        Y_Fuse = YCrCb_Fuse[:, 0:1, :, :]
        Cr_Fuse = YCrCb_Fuse[:, 1:2, :, :]
        Cb_Fuse = YCrCb_Fuse[:, 2:, :, :]

        R_vis = torchvision.transforms.functional.adjust_gamma(input_vis, 0.8, 1)
        YCrCb_R_vis = RGB2YCrCb(R_vis)
        Y_R_vis = YCrCb_R_vis[:, 0:1, :, :]
        Cr_R_vis = YCrCb_R_vis[:, 1:2, :, :]
        Cb_R_vis = YCrCb_R_vis[:, 2:, :, :]
        ...
        gradient_loss = 0.5 * self.l1_loss(
            Fuse_grad_x, joint_grad_x
        ) + 0.5 * self.l1_loss(Fuse_grad_y, joint_grad_y)
...
class MakeLoss(nn.Module):
    def __init__(self, background):
        ...
        self.semantic_loss = nn.CrossEntropyLoss(
            reduction="mean", ignore_index=background
        )
