...
class FusionlossAligned(nn.Module):
    ...
    并保留符号，相当于多了两条约束（方向+极性），对边缘形态更“苛刻”→ 更容易得到更锐的边缘与更贴源的纹理。

    细粒度匹配：同一像素里可以出现“x 跟 A、y 跟 B”，比“强度整体取 max 后再对齐”更自由也更强约束。

    代价与风险

    对齐敏感：源图间有微小位移/尺度差时，新版会因符号/方向不一致而产生更大惩罚，容易出现锯齿、振铃或细纹放大。

    噪声敏感：噪声会直接体现在方向分量里；强度范式对噪声相对更鲁棒。
