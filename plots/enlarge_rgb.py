...
if __name__ == '__main__':
    ...
    picture_index = ['20']
    position = [[203,184,238,266]]
    location=['top_left']
    ratio = [3]

    for i in range(len(picture_index)):

        readpath = r'D:\SOTA\outputs\lrrnet_rgb\{}.png'.format(picture_index[i])
        img = cv2.imread(readpath)  
        result = Partial_magnification(img, position[i], location=location[i], ratio=ratio[i])
        cv2.imwrite('D:/SOTA/visual_test_rgb/{}/lrrnet.png'.format(picture_index[i]), result)
